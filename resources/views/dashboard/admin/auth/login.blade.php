<x-dashboard.admin.layout.auth.app>
	...
	<div class="flex items-center justify-center grow bg-center bg-no-repeat page-bg">
		<div class="kt-card max-w-[370px] w-full">
			...
			<form action="{{ route('dashboard.admin.auth.login.store') }}" class="kt-card-content flex flex-col gap-5 p-10" id="sign_in_form" method="post">
				...
				<div class="flex flex-col gap-1">
					...
					<input class="kt-input" id="email" autocomplete="email" aria-invalid="{{ $errors->has('email') ? 'true' : 'false' }}" name="email" placeholder="email@email.com" value="{{ old('email', 'super_admin@app.com') }}"/>
					@error('email')
				...
				<div class="flex flex-col gap-1">
					<div class="flex items-center justify-between gap-1">
						...
						{{--<a class="text-sm kt-link shrink-0" href="html/demo1/authentication/classic/reset-password/enter-email.html">
							{{ trans('admin.auth.forgot_password') }}
						</a>--}}
					</div>
